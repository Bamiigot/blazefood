...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 06, 2023 at 07:12 PM
...
CREATE TABLE `orders` (
  ...
  `adress_latitude` decimal(9,6) NOT NULL,
  `adress_longitude` decimal(9,6) NOT NULL,
  `delivery_end_time` datetime NOT NULL,
  `status` enum('Готовиться','В пути','Доставлен') NOT NULL DEFAULT 'Готовиться'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `order_details`
  ...
  ADD CONSTRAINT `fk_order_detail_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
